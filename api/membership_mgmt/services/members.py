...
class MemberServices:
    ...
    async def create_new_member(self, member: MemberIn, branch_code: str):
        """Create New Member: accessible to only church admins."""
        try:
            # fetch and check if church is a branch
            church = await self.church_services.get_church_by_code(branch_code)
            if church.Level_Code != "BRN":
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Only Branch churches can have members",
                )
            ...
            validate_code_type(member.Join_Reason, "Exit/Join Reason", self.db)

            level = get_level(branch_code, self.current_user.HeadChurch_Code, self.db)

            # set user access
            set_user_access(
                self.current_user_access,
                headchurch_code=self.current_user.HeadChurch_Code,
                church_code=branch_code,
            ...
            self.db.execute(
                ...
                INSERT INTO tblMemberBranch
                    (Member_Code, Church_Code, Join_Date, Join_Reason, Join_Note)
                VALUES
                    (:Member_Code, :Church_Code, :Join_Date, :Join_Reason, :Join_Note);
                """
                ),
            ...
            self.db.commit()
            # fetch new member
            new_member = await self.get_member_by_code(new_code.Code)
            return new_member
        ...
        try:
            ...
            member = await self.db.execute(
                text(
                    ...
                    SELECT M.* , MC.Branch_Code, MC.Join_Date, MC.Join_Reason, MC.Join_Note FROM tblMember M
                    LEFT JOIN tblMemberBranch MC ON MC.Member_Code = M.Code
                    WHERE `Code` = :Code AND MC.Branch_Code = :Church_Code AND HeadChurch_Code = :HeadChurch_Code AND M.Is_Active = :Is_Active AND MC.Is_Active = :Is_Active;
                    """
                ),
                dict(
                    Code=member_code,
